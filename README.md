...
- Following steps:
    ...
    - [Done] 2: prepare a VM image (prepare.sh and copy_install_files.sh), then cp it for each instance (HDD) (07-deploy.sh)
    - [Done] 3: take a snapshot, then cp it for each instance (create_pre_snapshot.sh) (HDD) (08-deploy.sh)
